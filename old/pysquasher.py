...
def create_img(metadata):
    ...
    img = np.rot90(np.real(gfft.gfft(np.ravel(data), in_ax, out_ax, enforce_hermitian_symmetry=True, verbose=False, W=config.window, alpha=config.alpha)), 1)*mask
    print(img.shape)
    plt.imsave('test.pdf', img)
    exit()
